<template>
	<div class="p-6">
	  ...
	  <div v-else-if="error">
		<p class="text-red-500">Error al cargar el movimiento: {{ error }}</p>
	  </div>
  ...
  const { fetchMovement, handleFileUpload: uploadFile, updateMovementStatus } = useMovements();
  ...
  const fetchData = async () => {
	const { data, error: fetchError } = await fetchMovement(route.params.id);
	if (fetchError) {
  ...
  const handleFileUpload = async (event) => {
	...
	try {
	  const { data: uploadedFile, error: uploadError } = await uploadFile(file, route.params.id);
	  if (uploadError) throw uploadError;
  
	  if (uploadedFile.importeOperacion !== movement.value.ImporteDevolucion) {
		const createClarification = await fetch('https://gsict.azurewebsites.net/api/v1/cfs', {
		  method: 'POST',

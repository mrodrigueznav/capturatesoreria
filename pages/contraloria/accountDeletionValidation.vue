<template>
  <div class="p-6 border-2 border-green-500">
    <h1 class="text-2xl font-bold mb-6 text-white">Confirmación de Baja en Banca Electrónica</h1>
...
const fetchMovements = async () => {
  ...
  movements.value = data.filter(movement => movement.WorkflowStatus === 6);
};
...
const confirmValidation = async () => {
  try {
    const { success, error } = await updateMovementStatus(selectedMovement.value.id, 3);
